...
try:
    ...
    def _get_history(sid): raw = _redis_client.get(f"h:{sid}"); return _json.loads(raw) if raw else []
    def _set_history(sid, h): _redis_client.setex(f"h:{sid}", 86400, _json.dumps(h))
    def _init_history(sid):
        if not _redis_client.exists(f"h:{sid}"): _set_history(sid, [])
    SESSION_BACKEND = "redis"
except Exception as _e:
    logger.warning("Redis unavailable (%s) — in-memory session store", _e)
    _mem: dict[str, list[dict]] = {}
    def _get_history(sid): return _mem.get(sid, [])
    def _set_history(sid, h): _mem[sid] = h
    def _init_history(sid): _mem.setdefault(sid, [])
...
def _append_lead_to_sheet(lead: LeadCapture) -> bool:
    ...
    if sheet is None:
        return False
    try:
        sheet.append_row([lead.name, lead.phone, lead.session_id, lead.query or ""], value_input_option="USER_ENTERED")
        return True
    except Exception as exc:
...
scheduler.add_job(lambda: delete_old_collections(max_age_seconds=86400), "interval", hours=24)
...
async def lifespan(app: FastAPI):
    ...
    scheduler.start()
    logger.info("RAGbot backend started — env=%s session_backend=%s", settings.env, SESSION_BACKEND)
    yield
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[settings.cors_origin, "http://localhost:3000", "http://127.0.0.1:5500"],
...
# ─── Session validate ─────────────────────────────────────────────────────────
@app.get("/session/{session_id}", response_model=SessionValidateResponse, tags=["session"],
         dependencies=[Depends(require_api_key)])
async def validate_session(session_id: str):
    return SessionValidateResponse(valid=session_exists(session_id), session_id=session_id)
...
async def upload_pdf(request: Request, file: UploadFile = File(...)):
    ...
    if not validate_pdf_bytes(contents):
        raise HTTPException(status_code=415, detail="File is not a valid PDF (failed magic-byte check).")
    ...
    logger.info("PDF uploaded — session=%s pages=%d chunks=%d", session_id, page_count, chunk_count)
...
def push_progress(token: str, payload: str):
...
@app.post("/upload/streamed", tags=["document"], dependencies=[Depends(require_api_key)])
@limiter.limit("10/minute")
async def upload_pdf_streamed(request: Request, file: UploadFile = File(...),
    ...
    """
    def emit(stage: str, pct: int, detail: str = ""):
        import json
        push_progress(progress_token, json.dumps({"stage": stage, "pct": pct, "detail": detail}))

    if file.content_type not in ("application/pdf",):
        if progress_token: emit("error", 0, "Only PDF files are accepted.")
        raise HTTPException(status_code=415, detail="Only PDF files are accepted.")
    ...
    size_mb = len(contents) / (1024 * 1024)
    if size_mb > settings.max_file_size_mb:
        if progress_token: emit("error", 0, f"File exceeds {settings.max_file_size_mb} MB.")
        raise HTTPException(status_code=413, detail=f"File exceeds {settings.max_file_size_mb} MB limit.")

    emit("validating", 15)
    if not validate_pdf_bytes(contents):
        if progress_token: emit("error", 0, "Not a valid PDF.")
    ...
    try:
        ...
        os.remove(upload_path)
        if progress_token: emit("error", 0, str(exc))
        raise HTTPException(status_code=422, detail=str(exc))
    except Exception as exc:
        os.remove(upload_path)
        if progress_token: emit("error", 0, "Processing failed.")
    ...
    _init_history(session_id)
    logger.info("PDF uploaded (streamed) — session=%s pages=%d chunks=%d", session_id, page_count, chunk_count)
    return UploadResponse(
        success=True,
        message=f"Processed {page_count} page(s) into {chunk_count} searchable chunks.",
        session_id=session_id,
...
async def chat(request: Request, body: ChatRequest):
    ...
    return ChatResponse(reply=reply, lead_triggered=lead_triggered, source_chunks=source_chunks)
...
async def capture_lead(request: Request, lead: LeadCapture):
    ...
    return {"success": True, "message": "Thank you! A representative will be in touch shortly.",
            "sheet_saved": sheet_ok, "email_sent": email_ok}


# ─── Clear history ────────────────────────────────────────────────────────────
@app.delete("/session/{session_id}/history", tags=["session"], dependencies=[Depends(require_api_key)])
